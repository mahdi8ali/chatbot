...
export const TOOL_SEARCH_PROJECTS: ChatCompletionTool = {
  ...
  function: {
    ...
    description: "بحث في قاعدة بيانات شبكة الكفيل: الأخبار، سيرة أبي الفضل العباس (ع)، التاريخ، ومكتبة الفيديو. استخدم هذه الأداة لاسترجاع/إيجاد محتوى عن موضوع، بما في ذلك: أسماء المسؤولين والمناصب الإدارية (رئيس قسم، مدير، أمين عام)، الفعاليات، الإنجازات، والأخبار بشكل عام. النتائج مصنّفة بحقل source_label. ⚠️ لا تستعملها لأسئلة العدّ الكمّي («كم» / «عدد» / «كم مرة»)؛ فحقل total هنا تقديري للاسترجاع وليس عدّاً دقيقاً. إذا كان السؤال «كم خبراً عن <موضوع/كلمة>» فاستخدم count_mentions. ⚠️ إذا ذكر المستخدم شهر أو سنة محددة (مثل: حزيران 2026، يناير، الشهر الماضي) → استخدم from_date و to_date لتضييق النتائج على تلك الفترة فقط.",
    parameters: {
      type: "object",
      properties: {
        query: {
        ...
        from_date: {
          type: "string",
          description: "تاريخ البداية بصيغة YYYY-MM-DD (اختياري). استخدمه عندما يذكر المستخدم شهر أو فترة زمنية محددة."
        },
        to_date: {
          type: "string",
          description: "تاريخ النهاية بصيغة YYYY-MM-DD (اختياري). استخدمه مع from_date لتحديد نطاق زمني."
        },
        limit: {
          type: "number",
...
export const TOOL_COUNT_NEWS: ChatCompletionTool = {
  ...
  function: {
    name: "count_news",
    description: "عدّ إجمالي الأخبار المنشورة ضمن فترة زمنية و/أو تصنيف رقمي محدّد — **بلا موضوع أو كلمة بحث**. مثال: «كم خبراً نُشر خلال آخر 30 يوماً؟» أو «كم خبراً في القسم رقم X؟». ⚠️ إذا ذكر السؤال موضوعاً أو كلمة محدّدة (مثل «عن زيارة عرفة» أو «عن الأربعين») فلا تستخدم هذه الأداة — استخدم count_mentions.",
    parameters: {
      type: "object",
      properties: {
        category_id: {
