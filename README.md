...
> ## ⚠️ هذا الملف قديم — لا تعتمد عليه
> **المرجع المعتمد هو [`PROJECT-AUDIT.md`](./PROJECT-AUDIT.md)** (دراسة مبنية على قراءة الشيفرة الفعلية).
>
> **حُذف الودجت الجانبي الصغير بتاريخ 2026-08-10** ومعه `public/widget.js` و`/api/widget`
> و`preview.html` ومجلّد `tests/`. كل تعليمات «التضمين السريع» أدناه **لم تعد تعمل**.
>
> المشروع الآن هو **الودجت الكامل** الذي يُفتح ملء الشاشة:
> `npm run dev` ثم `/ar` ← زر «المساعد الذكي». الشيفرة في `components/chat/*`.
> التفاصيل في [`PROJECT-AUDIT.md` §0.1](./PROJECT-AUDIT.md).
