...
/**
 * صفحة التحليلات — admin/page.tsx
 *
 * منقولة حرفياً من app/[locale]/analytics/page.tsx مع تعديلين فقط:
 *  1) تُعرض داخل قشرة الإدارة (layout يوفّر الترويسة والتنقّل)، لذا أُزيل شريط
 *     الترويسة الداخلي (navy + العنوان) والخط الفاصل — بقيت وظيفة التحديث في
 *     شريط أدوات صغير أعلى المحتوى.
 *  2) لوحة الألوان C وأنماط الجدول thStyle/tdStyle والمكوّنات StatCard/Badge/
 *     EmptyState تُستورد من "./_shared" بدل إعادة تعريفها محلياً.
 *
 * سلوك جلب البيانات من /api/analytics يعتمد كوكي جلسة الإدارة تلقائياً؛ عند 401
 * يُعاد التوجيه إلى /{locale}/admin/login. RTL وخط 'Readex Pro' محفوظان.
 */
...
import { C, thStyle, tdStyle, StatCard, Badge, EmptyState } from "./_shared"
...
export default function AnalyticsPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (res.status === 401) { window.location.href = `/${locale}/admin/login`; return }
  ...
  return (
    <div style={{ fontFamily: "'Readex Pro','Segoe UI',sans-serif", direction: "rtl", color: C.text }}>
