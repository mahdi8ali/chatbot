...
import { update, setActive, remove } from "@/lib/server/kb-service"
...
/**
 * PUT /api/knowledge/[id] — تحديث جزئي لمقالة.
...
export async function PUT(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    const result = validateKbInput(body, { partial: true })
    if (!result.ok) {
    ...
    console.error("[KB API PUT]", err)
    return Response.json({ error: err.message }, { status: 500 })
  }
}

/**
 * PATCH /api/knowledge/[id] — تبديل حالة التفعيل فقط.
 * محمي بـ requireAdmin قبل أي منطق.
 */
export async function PATCH(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    console.error("[KB API PATCH]", err)
    return Response.json({ error: err.message }, { status: 500 })
  }
}

/**
 * DELETE /api/knowledge/[id] — حذف مقالة.
 * محمي بـ requireAdmin قبل أي منطق.
 */
export async function DELETE(req: Request, ctx: Ctx) {
  ...
  try {
    ...
    console.error("[KB API DELETE]", err)
    return Response.json({ error: err.message }, { status: 500 })
  }
}
